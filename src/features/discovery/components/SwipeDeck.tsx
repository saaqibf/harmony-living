...
export function SwipeDeck({ initialProfiles, swipesRemaining }: Props) {
  ...
  if (matchBanner) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#A86472] to-[#1A0A14] flex flex-col items-center justify-center px-6 text-center">
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs">
          {matchBanner.conversationId && (
            <Link
              ...
              className="bg-white text-[#A86472] font-semibold py-3.5 rounded-xl text-center hover:bg-[#F9F0EE] transition-colors text-sm"
            >
              Send a message
            </Link>
          )}
  ...
  if (remaining === 0) {
    return (
      <div className="min-h-screen bg-[#F2E6E0] flex flex-col items-center justify-center px-6 text-center">
        ...
        <Link
          href="/matches"
          className="bg-[#A86472] text-white font-semibold px-6 py-3 rounded-xl hover:bg-[#8A505E] transition-colors text-sm"
        >
          View my matches
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#F2E6E0]">
      ...
      <div className="px-6 pt-6 pb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-[#F9F0EE] border border-[#E8D5D0] rounded-full px-3.5 py-1.5 text-xs font-semibold text-[#A86472]">
        ...
        {FILTER_CHIPS.map((chip) => (
          <button
            ...
            className={`shrink-0 text-xs font-medium px-4 py-1.5 rounded-full border transition-colors ${
              activeFilter === chip
                ? 'bg-[#A86472] text-white border-[#A86472]'
                : 'bg-white text-[#4c4640] border-[#cfc5bd] hover:border-[#A86472]'
            }`}
          >
            {chip}
          </button>
      ...
      <div className="px-6 pb-6">
        <div className="bg-white rounded-[14px] border border-[#cfc5bd] overflow-hidden shadow-sm">
          {/* Photo */}
          <div className="relative h-80 bg-[#EFE0D8]">
            {showPhoto ? (
              <img
          ...
          <div className="p-5 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="font-mono text-xs font-bold text-[#A86472]">{scorePercent}%</span>
              </div>
              <div className="h-1.5 bg-[#EFE0D8] rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#A86472] rounded-full transition-all"
            ...
            <div className="flex flex-wrap gap-1.5">
              {current.faith && (
                <span className="text-xs px-2.5 py-1 rounded-full bg-[#F9F0EE] text-[#A86472] border border-[#E8D5D0]">{current.faith}</span>
              )}
              {current.gender && (
                <span className="text-xs px-2.5 py-1 rounded-full bg-[#F9F0EE] text-[#A86472] border border-[#E8D5D0]">{current.gender}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              <button
                ...
                disabled={isPending}
                aria-label="Pass"
                className="flex-1 flex items-center justify-center gap-2 py-3 border border-[#cfc5bd] rounded-xl text-[#7d766f] hover:bg-[#F9F0EE] hover:border-[#A86472] hover:text-[#A86472] disabled:opacity-40 transition-all text-sm font-medium"
              >
              ...
              <button
                ...
                disabled={isPending}
                aria-label="Connect"
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#A86472] rounded-xl text-white hover:bg-[#8A505E] disabled:opacity-40 transition-all text-sm font-semibold"
              >
