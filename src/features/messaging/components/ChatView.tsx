    ...
    <div className="flex flex-col h-full bg-gray-50">
              ...
              <div className={`flex flex-col gap-0.5 max-w-[72%] ${mine ? 'items-end' : 'items-start'}`}>
                <div
                  className={`px-4 py-2.5 text-[14px] leading-relaxed ${
                    mine
                      ? 'bg-teal-600 text-white rounded-2xl rounded-br-[6px]'
      ...
      <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center gap-3">
        <input
          ...
          className="flex-1 rounded-full border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-500/20 transition-colors"
        />
        <button
          onClick={send}
          disabled={isPending || !body.trim()}
          aria-label="Send message"
          className="w-10 h-10 rounded-full bg-teal-600 flex items-center justify-center text-white hover:bg-teal-700 active:scale-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shrink-0"
        >
