...
export default function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen">
      {/* Left panel — brand */}
      <div className="hidden md:flex md:w-[420px] lg:w-[480px] bg-gradient-to-br from-[#7B2D5C] via-[#5A1F43] to-[#1A0A14] flex-col justify-between px-10 py-12 shrink-0">
        ...
        <div>
          <p className="font-serif text-white text-3xl lg:text-4xl font-semibold leading-[1.2] mb-8">
            Find a home<br />where you truly<br /><em className="not-italic text-[#c28aab]">belong.</em>
          </p>
          <ul className="space-y-4">
            {[
              ...
              <li key={item} className="flex items-start gap-3 text-white/75 text-sm leading-relaxed">
                <span className="w-1.5 h-1.5 rounded-full bg-[#c28aab] mt-1.5 shrink-0" />
                {item}
              </li>
            ))}
          </ul>
