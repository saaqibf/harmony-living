            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 transition-colors relative ${
                  active
                    ? 'bg-[#F0E4E0] border-l-[3px] border-[#A86472]'
                    : 'border-l-[3px] border-transparent hover:bg-[#F5EAE4]'
                }`}
              >
                <div className="relative shrink-0">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-[#F5EAE4] flex items-center justify-center">
                    {c.otherUser.photoUrl ? (
                      <img src={c.otherUser.photoUrl} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-xl text-[#A86472]">
                  ...
                  {c.hasUnread && (
                    <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-[#A86472] border-2 border-[#fdfafc]" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    {c.lastMessage && (
                      <span className={`text-xs shrink-0 ml-1 ${c.hasUnread ? 'text-[#A86472] font-medium' : 'text-[#9d9097]'}`}>
