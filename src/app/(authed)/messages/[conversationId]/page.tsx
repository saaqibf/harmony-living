...
import { ListingContextBanner } from '@/features/messaging/components/ListingContextBanner';
...
export default async function ConversationPage({
  ...
  const listing = conversation?.listing
    ? { ...conversation.listing, coverImageUrl: conversation.listing.images[0]?.url ?? null }
    : null;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-4 px-6 py-3.5 bg-white border-b border-[#e8e0e5] shrink-0">
        <div className="w-12 h-12 rounded-full overflow-hidden bg-[#F5EAE4] shrink-0 ring-2 ring-[#e8d0e2]">
          {otherPhotoUrl ? (
            <img src={otherPhotoUrl} alt={otherFirstName} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-[#A86472]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#F0E4E0]">
              <svg className="w-3 h-3 text-[#A86472]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
              </svg>
              <span className="text-[10px] font-semibold text-[#A86472] tracking-wide">Verified Match</span>
            </div>
          </div>
        </div>
        ...
        {/* Action icons */}
        <div className="flex items-center gap-1">
          <button className="p-2.5 rounded-full hover:bg-[#F5EAE4] transition-colors text-[#7d766f] hover:text-[#A86472]">
      ...
      {/* Listing context banner, shown when this conversation was started from a listing */}
      {listing && <ListingContextBanner listing={listing} />}
